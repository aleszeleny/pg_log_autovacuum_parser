...
def parse_autovacuum(autovacuum_log):
    log_timestamp = autovacuum_log["timestamp"]
    ...
    if autovacuum_data:
        print(
                '%s\t%s.%s.%s: index scans: %s, pages: ( %s removed, %s remain, %s scanned - %s %% of total), tuples: ( %s removed, %s remain, %s dead not removable )' % (
